    ...
    def _run(self):
        ...
        for station in self.stations.values():
            ...
            if self.sync_mode in ['spec', 'both']:
                local_dir = str(
                    f'{self.res_dir}/{self.analysis_date}/'
        ...
        if nscans.any():
            ...
            flux_results = calculate_fluxes(
                ...
                scans=all_scans,
                fpath=fpath,
                vent_loc=self.volc_loc,
                default_alt=self.default_alt,
                default_az=self.default_az,
                wind_speed=self.wind_speed,
                scan_pair_time=self.scan_pair_time,
                scan_pair_flag=self.scan_pair_flag,
                min_scd=self.min_scd,
                max_scd=self.max_scd,
